...
import { FontaineItem } from '@/data/theoremeFontaineGame';

type Phase = 'intro' | 'playing' | 'feedback';

const HS_KEY = 'theoreme_fontaine_high_score';
...
export default function TheoremeFontaineQuizGame({ items }: { items: FontaineItem[] }) {
  const [deck, setDeck] = useState<FontaineItem[]>(() => shuffle(items));
  ...
  const handleGuess = useCallback(
    (guessedA: boolean) => {
      ...
      const correct = guessedA === current.isOptionA;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center" style={{ background: bg }}>
        <div className="z-10 bg-white/95 rounded-3xl p-10 shadow-xl max-w-md w-full">
          ...
          {highScore > 0 && (
            <div className="rounded-2xl px-4 py-3 mb-6 inline-block" style={{ background: 'linear-gradient(90deg, rgba(37,99,235,0.12), rgba(234,88,12,0.12))' }}>
          ...
          <div className="p-5">
            {phase === 'playing' ? (
              ...
              <div className="py-2 text-center">
                <p className="text-green-500 font-black text-xl">Correct ! +1 ✓</p>
...
const bg = 'linear-gradient(135deg, #1E3A8A 0%, #6D28D9 50%, #EA580C 100%)';
